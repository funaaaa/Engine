...
void StageObjectMgr::Setting()
{
	...
	for (auto& index : forRankingWalls) {
...
int StageObjectMgr::AddRankingWall(const std::string& DirectryPath, const std::string& ModelName, const std::wstring& HitGroupName)
{
	...
	for (auto& index : forRankingWalls) {
		...
		addIndex = static_cast<int>(&index - &forRankingWalls[0]);
	...
	// オブジェクトを設定。
	forRankingWalls[addIndex].first->Setting(BaseStageObject::OBJECT_ID::RANKING_WALL, BaseStageObject::COLLISION_ID::MESH, instance);
	forRankingWalls[addIndex].second = true;
...
BaseStage::ColliderOutput StageObjectMgr::Collider(BaseStage::ColliderInput Input)
{
	...
	output.rankingWallIndex_ = 0;
	output.isHitRankigWall_ = false;
	...
	// 通常オブジェクトとの当たり判定
	CheckHitBasicObject(Input, output);

	// ランキング計測用壁との当たり判定
	CheckHitRankingWallObject(Input, output);
...
void StageObjectMgr::AddScaleRankingWall(int Index, const Vec3& Scale)
{

	/*===== スケールを加算 =====*/

	forRankingWalls[Index].first->AddScale(Scale);
...
void StageObjectMgr::NonDisplayRankingWall(int Index)
{

	/*===== 非表示 =====*/

	forRankingWalls[Index].first->NonDisplay();
...
BaseStage::ColliderOutput StageObjectMgr::OrnamentMeshCollider(BaseStage::ColliderInput& Input, FHelper::RayToModelCollisionData InputRayData, BaseStage::ColliderOutput Output, BaseStageObject::OBJECT_ID ObjectID)
{
	...
	if (isHit) {
		...
		else if (ObjectID == BaseStageObject::OBJECT_ID::RANKING_WALL) {
			Output.isHitRankigWall_ = true;
		}
		else {
			// 法線方向に当たった分押し戻す。
			Input.targetPos_ = impactPos + hitNormal * hitDistance;
...
void StageObjectMgr::CheckHitBasicObject(BaseStage::ColliderInput& Input, BaseStage::ColliderOutput& Output)
{
...
void StageObjectMgr::CheckHitRankingWallObject(BaseStage::ColliderInput& Input, BaseStage::ColliderOutput& Output)
{

	/*===== ランキング計測用オブジェクトとの当たり判定 =====*/
	...
	for (auto& index : forRankingWalls) {
		...
		// 当たり判定がMESHだったら。
		if (indexCollisionID == BaseStageObject::COLLISION_ID::MESH) {
			...
			// オブジェクトごとに当たり判定を行う
			bool isHitRankingWall = Output.isHitRankigWall_;
			Output = OrnamentMeshCollider(Input, InputRayData, Output, indexObjID);

			// ランキング計測用壁にあたったトリガーだったらインデックスを保存する。
			if (!isHitRankingWall && Output.isHitRankigWall_) {
				Output.rankingWallIndex_ = static_cast<int>(&index - &forRankingWalls[0]);
			}
