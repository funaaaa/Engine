...
WindowsAPI ImGuiWindow::windowsAPI{};
ComPtr<ID3D12Debug> ImGuiWindow::debugController{};
ComPtr<ID3D12Device5> ImGuiWindow::dev{};
ComPtr<IDXGIFactory6> ImGuiWindow::dxgiFactory{};
vector<ComPtr<IDXGIAdapter1>> ImGuiWindow::adapters{};
ComPtr<IDXGIAdapter1> ImGuiWindow::tmpAdapter{};
vector<D3D_FEATURE_LEVEL> ImGuiWindow::levels{};
D3D_FEATURE_LEVEL ImGuiWindow::featureLevel{};
ComPtr<IDXGISwapChain4> ImGuiWindow::swapchain{};
ComPtr<ID3D12CommandAllocator> ImGuiWindow::cmdAllocator{};
ComPtr<ID3D12GraphicsCommandList4> ImGuiWindow::cmdList{};
ComPtr<ID3D12CommandQueue> ImGuiWindow::cmdQueue{};
ComPtr<ID3D12DescriptorHeap> ImGuiWindow::rtvHeaps{};
ComPtr<ID3D12Resource> ImGuiWindow::depthBuffer{};
ComPtr<ID3D12DescriptorHeap> ImGuiWindow::dsvHeap{};
vector<ComPtr<ID3D12Resource>> ImGuiWindow::backBuffers{};
D3D12_DESCRIPTOR_HEAP_DESC ImGuiWindow::heapDesc{};
ComPtr<ID3D12Fence> ImGuiWindow::fence{};
UINT64 ImGuiWindow::fenceVal{};
IDirectInput8* ImGuiWindow::dinput{};
IDirectInputDevice8* ImGuiWindow::devkeybord{};
IDirectInputDevice8* ImGuiWindow::devmouse{};
ComPtr<ID3D12DescriptorHeap> ImGuiWindow::heapForImgui{};
...
void ImGuiWindow::processBeforeDrawing() {
	//メッセージ確認
	if (PeekMessage(&windowsAPI.msg, nullptr, 0, 0, PM_REMOVE)) {
		TranslateMessage(&windowsAPI.msg);	//キー入力メッセージの処理
		DispatchMessage(&windowsAPI.msg);	//プロシージャにメッセージを送る
...
void ImGuiWindow::processAfterDrawing() {
	ImGui::End();
	ImGui::Render();
	////コマンドリストに追加
...
void ImGuiWindow::SetRenderTarget()
{
	//レンダーターゲットのリソースバリア変更
	UINT bbIndex = ImGuiWindow::Ins()->swapchain->GetCurrentBackBufferIndex();
	ImGuiWindow::Ins()->cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(ImGuiWindow::Ins()->backBuffers[bbIndex].Get(),
		D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
	ImGuiWindow::Ins()->cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(ImGuiWindow::Ins()->backBuffers[bbIndex].Get(),
		D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET));
	//レンダーターゲットの設定
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtvH = CD3DX12_CPU_DESCRIPTOR_HANDLE(
		ImGuiWindow::Ins()->rtvHeaps->GetCPUDescriptorHandleForHeapStart(), bbIndex, ImGuiWindow::Ins()->dev->GetDescriptorHandleIncrementSize(ImGuiWindow::Ins()->heapDesc.Type));
	ImGuiWindow::Ins()->cmdList->OMSetRenderTargets(1, &CD3DX12_CPU_DESCRIPTOR_HANDLE(ImGuiWindow::Ins()->rtvHeaps->GetCPUDescriptorHandleForHeapStart(),
		ImGuiWindow::Ins()->swapchain->GetCurrentBackBufferIndex(), ImGuiWindow::Ins()->dev->GetDescriptorHandleIncrementSize(ImGuiWindow::Ins()->heapDesc.Type)), false, &ImGuiWindow::Ins()->dsvHeap->GetCPUDescriptorHandleForHeapStart());
	//レンダーターゲットのクリア
	float clearColor[] = { 0.5f,0.5f,0.5f,0.0f };
	ImGuiWindow::Ins()->cmdList->ClearRenderTargetView(rtvH, clearColor, 0, nullptr);
	//深度バッファのクリアコマンド
	ImGuiWindow::Ins()->cmdList->ClearDepthStencilView(ImGuiWindow::Ins()->dsvHeap->GetCPUDescriptorHandleForHeapStart(), D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);
}

void ImGuiWindow::ResourceBarrierAfter()
{
	//レンダーターゲットのリソースバリア変更
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(backBuffers[swapchain->GetCurrentBackBufferIndex()].Get(),
		D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
...
ComPtr<ID3D12DescriptorHeap> ImGuiWindow::CreateDescriptorHeaoForImgui()
{
	ComPtr<ID3D12DescriptorHeap> ret;
