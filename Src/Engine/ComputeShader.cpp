...
void ComputeShader::Dispatch(UINT ThreadGroupCountX, UINT ThreadGroupCountY, UINT ThreadGroupCountZ)
{
	...
	Engine::Ins()->cmdList_->SetPipelineState(pipline_.Get());
...
void ComputeShader::GenerateRootSignature()
{
	...
	auto hr = Engine::Ins()->dev_->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&rootSignature_));
	if (FAILED(hr)) {
		// ルートシグネチャの作成に失敗した。
		assert(0);
	}
...
void ComputeShader::GeneratePipline(Microsoft::WRL::ComPtr<ID3DBlob> csBlob)
{
	...
	auto hr = Engine::Ins()->dev_->CreateComputePipelineState(&psoDesc, IID_PPV_ARGS(&pipline_));
	if (FAILED(hr)) {
		// 生成に失敗した
		assert(0);
	}
...
void ComputeShader::CommitDescHeap()
{
	...
	// ディスクリプタヒープを生成
	auto hr = Engine::Ins()->dev_->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&descHeap_));
	if (FAILED(hr)) {
	...
	// 次に進める。
	cpuHandle.ptr += Engine::Ins()->dev_->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	...
	gpuHandle.ptr += (UINT64)Engine::Ins()->dev_->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
