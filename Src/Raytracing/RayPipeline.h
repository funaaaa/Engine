...
protected:
	...
	std::vector<std::shared_ptr<RayPipelineShaderData>> shaderData_;			// 使用するシェーダーを纏めた構造体
...
public:
	...
	// セッティング処理
	void Setting(const std::vector<std::shared_ptr<RayPipelineShaderData>>& InputData, const int& UseHitGroup, const int& SRVCount, const int& CBVCount, const int& UAVCount, const int& PayloadSize, const int& AttribSize, const int& ReflectionCount = 4);
