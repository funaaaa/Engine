...
void RayEngine::Draw()
{
	...
	// DenoiseQueueが実行可能状態だったら。
	{
		...
		// AO情報にデノイズをかける。
		{
			...
			// AOにデノイズをかける。
			Denoiser::Ins()->Denoise(aoOutput_[Engine::Ins()->currentQueueIndex_]->GetUAVIndex(), denoiseAOOutput_[Engine::Ins()->currentQueueIndex_]->GetUAVIndex(), denoiseMaskOutput_[Engine::Ins()->currentQueueIndex_]->GetUAVIndex(), 1, 8);
		}
