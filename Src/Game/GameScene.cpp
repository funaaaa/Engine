...
void GameScene::Update()
{
	...
	constBufferData.lock()->debug_.seed_ = FHelper::GetRand(0, 1000);
	...
	// 太陽の角度を更新。
	sunAngle_ += sunSpeed_;
	if (0.0f < constBufferData.lock()->light_.dirLight_.lihgtDir_.y_) {
	...
	constBufferData.lock()->light_.dirLight_.lihgtDir_ = Vec3(-cos(sunAngle_), -sin(sunAngle_), 0.5f);
	constBufferData.lock()->light_.dirLight_.lihgtDir_.Normalize();
	// 天球自体も回転させる。
