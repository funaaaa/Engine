...
Texture2D<float4> shadowMapTex : register(t1); //1番スロットに設定されてテクスチャ
...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    float zInLVP = input.svposInLVP.z / input.svposInLVP.w;
    ...
    if (shadowMapUV.x > 0.0f && shadowMapUV.x < 1.0f && shadowMapUV.y > 0.0f && shadowMapUV.y < 1.0f)
    {
        ...
        // 計算したUVを使って、シャドウマップから深度値をサンプリング
        float zInShadowMap = shadowMapTex.Sample(smp, shadowMapUV).r;
   ...
   // }
    ...
    float4 returnBuff = (shadeColor * texcolor) * color;

    // 01の範囲に収める。
    saturate(returnBuff);

    returnBuff.xyz *= shadowMap;
    return returnBuff;
